...
async def extract_raw_table_results_data(page):
...
def extract_naplan_results(raw_table_html, calendar_year):
    """Parses the raw NAPLAN results HTML & returns a cleaned dataframe.

    Args:
        raw_table_html (_type_): _description_
        calendar_year (_type_): _description_

    Returns:
        _type_: DataFrame with columns:
    ...
    table = soup.find('table', {'id': 'similarSchoolsTable', 'class': 'naplan-result-table'})
    ...
    # The data is in row base, rather than column based format. Thus we need to repeat the table headers for each year level
